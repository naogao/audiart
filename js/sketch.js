...
function searchAudioFeatures() {
 s.getAudioFeaturesForTrack(songID, function(err, data) {
	 if (err) console.error(err)
	   ...
	   ColH = mySong.key                //  Set KEY to coresponding HUE (See top table)
	   ColS = mySong.key   //  Set VALENCE to SATURATION (rounded)
	   ColB = mySong.key    //  Set ENERGY to BRIGHTNESS (rounded)
	   radiusSize = mySong.key     //  Set TEMPO to RADIUS (rounded)

	   // ColH = mySong.key                //  Set KEY to coresponding HUE (See top table)
	   // ColS = mySong.valence   //  Set VALENCE to SATURATION (rounded)
	   // ColB = mySong.energy    //  Set ENERGY to BRIGHTNESS (rounded)
